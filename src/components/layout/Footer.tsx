...
export default function Footer() {
  ...
  const footer = config.footer || {
    ...
    aboutText: company.tagline || "Your journey. Our expertise. From quick getaways to international holidays, family vacations to special occasions — travel made memorable.",
    phone: company.phone || "+91 74069 94752",
  ...
  const waUrl = social.whatsapp || `https://wa.me/${cleanWa.startsWith('91') ? cleanWa : '91' + cleanWa}`;
        ...
        <div className={styles.top}>
          <div className={styles.brand}>
            ...
            <div className={styles.contactDetails}>
              {(footer.email || company.email) && (
                <div className={styles.contactItem}>
                  ...
                  <a href={`mailto:${footer.email || company.email}`} className={styles.contactLink}>
                    {footer.email || company.email}
                  </a>
                </div>
              )}
              ...
              {(footer.phone || company.phone) && (
                <div className={styles.contactItem}>
                  ...
                  <a href={`tel:${(footer.phone || company.phone).replace(/[^0-9+]/g, '')}`} className={styles.contactLink}>
                    {footer.phone || company.phone}
                  </a>
                </div>
              )}
              ...
              {(footer.address || company.address) && (
                <div className={styles.addressBox}>
                  ...
                  <span>{footer.address || company.address}</span>
                </div>
              )}

              {footer.workingHours && (
                <div className={styles.contactItem} style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)', marginTop: '0.2rem' }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: '#d4af37' }}>
            ...
            <div className={styles.socialWrapper}>
              <span className={styles.socialHeading}>{footer.socialHeading || "Connect With Us"}</span>
